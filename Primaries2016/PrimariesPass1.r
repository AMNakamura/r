...
# Wisconsin
wiR <- fromJSON("http://data.cnn.com/ELECTION/2016primary/WI/county/R.json",
                flatten=TRUE )
wiD <- fromJSON("http://data.cnn.com/ELECTION/2016primary/WI/county/D.json",
                flatten=TRUE)           
...
nvdem2 <- merge(x = nvdemVpct1, y = nvdemVnum1, by = "co_id", all.x = TRUE) 

#Wisconsin
wid1 <-melt(wiD$counties$race.candidates, id=1:15) ##Yes!85 obs. 16 variables. Yes! What does ID mean? 
wid2 <-wiD$counties[1:2] # co_id and (county) name

wid2$L1<-seq.int(nrow(wid2)) # Add a sequence number. Will merge with
# the L1 variable created by the melt() . Will then perform a 1:many
# join.

widem <- merge(x = wid1, y = wid2, by = "L1", all.x = TRUE) #Left outer join.

# Use dcast to transpose by the county name, code, and candidate
widemVpct1 <-dcast(widem, L1 + name + co_id ~ lname , value.var="pctDecimal")
widemVnum1 <-dcast(widem, co_id ~ lname , value.var="votes")

widemVpct1<-transform(widemVpct1, Clinton = as.numeric(Clinton)) #Cowiert to numeric
widemVpct1<-transform(widemVpct1, Sanders = as.numeric(Sanders)) #Cowiert to numeric

widem2 <- merge(x = widemVpct1, y = widemVnum1, by = "co_id", all.x = TRUE) #Left outer join.
...
# Create a file with the percentages and vote counts in one file.
nvrep2 <- merge(x = nvrepVpct1, y = nvrepVnum1, by = "co_id", all.x = TRUE) 

#Wisconsin
wir1 <-melt(wiR$counties$race.candidates, id=1:15) ##Yes!30 obs. 16 variables. Yes! What does ID mean? 
wir2 <-wiR$counties[1:2] # co_id and county name

wir2$L1<-seq.int(nrow(wir2)) # Add a sequence number. Will merge with
# the L1 variable created by the melt() . Will then perform a 1:many
# join.

wirep <- merge(x = wir1, y = wir2, by = "L1", all.x = TRUE) #Left outer join.

# Use dcast to transpose by the county name, code, and candidate
wirepVpct1 <-dcast(wirep, L1 + name + co_id ~ lname , value.var="pctDecimal")
wirepVnum1 <-dcast(wirep, co_id ~ lname , value.var="votes")

# Bush gone. Down to Trump, Carson, Cruz, Rubio, and Kasich
wirepVpct1<-transform(wirepVpct1, Cruz = as.numeric(Cruz)) # to numeric
wirepVpct1<-transform(wirepVpct1, Kasich = as.numeric(Kasich)) # to numeric
wirepVpct1<-transform(wirepVpct1, Trump = as.numeric(Trump)) # to numeric

# Create a file with the percentages and vote counts in one file.
wirep2 <- merge(x = wirepVpct1, y = wirepVnum1, by = "co_id", all.x = TRUE) 
...
# Wisconsin
widem2 <- azdem2[order(widem2$L1),]
widem2$tvotes <- rowSums(widem2[c(6:7)], na.rm = TRUE)
widem2$first <- do.call(pmax, widem2[6:7])
widem2$second <- do.call(pmin, widem2[6:7])
widem2$minVotes <- ceiling((widem2$first - widem2$second)/2)
widem2$margin <- round((widem2$minVotes)/widem2$tvotes,digits=2)
summary(widem2$margin)

ties <-subset(widem2, widem2$first==widem2$second) 
ties 

# Find the name of the winner. This is simple when only two candidates are running.
widem2$Winner <- ifelse(widem2$Sanders.y > widem2$Clinton.y,'Sanders','Clinton')
# ties
widem2$Winner <- ifelse(widem2$first==widem2$second,'Tied',widem2$Winner)
...
# Wisconsin
wirep2 <- wirep2[order(wirep2$L1),]
rownames(wirep2) <- NULL
wirep2$tvotes <- rowSums(wirep2[,grepl(".y", names(wirep2))], na.rm = TRUE)
wirep2$first <- do.call(pmax, wirep2[,grepl(".y", names(wirep2))])
wirvotes <-wirep2[,grepl(".y", names(wirep2))]

wirWin <-as.data.frame(cbind(row.names(wirvotes),apply(wirvotes,1,function(x)
  names(wirvotes)[which(x==max(x))])))
# V1 is a factor, so convertert to numeric before trying to merge.
#if you don't convert to character first, you'll get the underlying levels.
wirWin$L1 = as.numeric(as.character(wirWin$V1)) 
names(wirWin)[names(wirWin)=="V2"] <- "Winner"
wirWin <-subset(wirWin,select=c(Winner,L1))
wirWin$Winner <-as.character(wirWin$Winner)

# find the second highest number of votes;
wir2nd <- as.data.frame(cbind(row.names(wirvotes),apply(wirvotes, 1, function(x) sort(x, decreasing=TRUE)[2]) 
))
wir2nd$L1 = as.numeric(as.character(wir2nd$V1)) #convert to numeric. 

names(wir2nd)[names(wir2nd)=="V2"] <- "second"
# Convert second from a factor to numeric. 
wir2nd$second <- as.numeric(as.character(wir2nd$second))

wirk <- merge(x = wir2nd, y = wirWin, by = "L1") #winner and 2nd highest vote count.
wirk$Winner<-sub(pattern = ".y", replacement = "",  x = wirk$Winner) 

wirep3 <- merge(x = wirk, y = wirep2, by = "L1") #Merge with source
wirep3$minVotes <- ceiling((wirep3$first - wirep3$second)/2)
wirep3$margin <- round((wirep3$minVotes)/wirep3$tvotes,digits=2)
summary(wirep3$margin)

ties <-subset(wirep3, wirep3$first==wirep2$second) 
ties 

ties <- subset(wirep3,wirep3$minVotes == 0)
wirep3$Winner <- ifelse(wirep3$minVotes == 0,"Tied (Cruz,Trump)",wirep3$Winner)
...
wirtabl <-aggregate(first~margin+Winner, data=wirep3, sum, na.rm=TRUE)
wirbubble=aggregate(tvotes~margin+Winner, data=wirep3, sum, na.rm=TRUE)
wirtabl<- merge(x = wirtabl, y = wirbubble, by = c("margin","Winner"))
wirtabl$radius <- sqrt( wirtabl$tvotes/ pi ) 
...
# Democrats - Wisconsin
widtabl <-aggregate(first~margin+Winner, data=widem2, sum, na.rm=TRUE)
widbubble=aggregate(tvotes~margin+Winner, data=widem2, sum, na.rm=TRUE)
widtabl<- merge(x = widtabl, y = widbubble, by = c("margin","Winner"))
widtabl$radius <- sqrt( widtabl$tvotes/ pi ) 
...
nhrepBar<-ggplot(nhrtabl, aes(x=margin, y=first,fill=Winner)) + 
  ...
  scale_fill_manual(values=c("Cruz"="red", "Trump"="darkorange", "Kasich"="maroon",
...
# Bar-Charts Wisconsin
votes<-as.character(format(sum(wirep3$tvotes),big.mark=","))
plot.title <-"Wisconsin Republican Primary Presidential Race"
plot.subtitle <-paste("Total votes: ", votes)

wirepBar<-ggplot(wirtabl, aes(x=margin, y=first,fill=Winner)) + 
  geom_bar(stat="identity") + scale_x_continuous(limits=c(0,.5)) +
  scale_y_continuous(labels = comma, limits=c(0,25000)) +
  xlab("Margin of Victory") +
  ylab("Votes Cast for Winner") +
...
votes<-as.character(format(sum(widem2$tvotes),big.mark=","))
plot.title <-"Wisconsin Democratic Primary Presidential Race"
plot.subtitle <-paste("Total votes: ", votes)

widemBar <-ggplot(widtabl, aes(x=margin, y=first,fill=Winner)) + 
  geom_bar(stat="identity") + scale_x_continuous(limits=c(0,.5)) +
  scale_y_continuous(labels = comma, limits=c(0,25000)) +
  xlab("Margin of Victory") +
  ylab("Votes Cast for Winner") +
...
grid.arrange(widemBar, wirepBar,ncol = 2)
...
save(wirtabl,file="wirtabl.Rda")
save(widtabl,file="widtabl.Rda")
